        ...
        try:
            ...
            try:
                ...
                for pk3 in pk3s:
                    ...
                    try:
                        ...
                        bsps += [basename(bsp)[:-4] for bsp in iter(namelist(pk3zip)) if
                                 endswith(lower(bsp), ".bsp")]  # Get all BSP files.
                        pk3close(pk3zip)
            ...
            if self.roundlimit or self.timelimit or self.rtv:
                                ...
                                try:
                                    ...
                                    if self.secondary_maps:
                                        ...
                                        repeated_maps = tuple((mapname for mapname in iter(self.secondary_maps)
                                                               if lower(
                                            mapname) in repeated_maps))  # Compare primary and secondary maps.
                                    ...
                                    if errno == 2:
                                        ...
                                        raise Exception(
                                            "Unexpected error while reading secondary map file (ERRNO: %i)." % (errno))
                ...
                if (len(self.maps) + len(self.secondary_maps)) > 5:
                    ...
                    raise Exception(
                        "Pick secondary maps must be enabled whether secondary maps are present and nominations are disabled.")
        ...
        try:
            ...
            try:
                ...
                bsps = tuple((bsp for bsp in iter(bsps)
                              if (lower(bsp) not in map_duplicates and
                                  not map_duplicates[
                                      lower(bsp)])))  # Ensure that we don't get duplicates from replacements.
                lower_bsps = tuple((lower(bsp) for bsp in iter(bsps)))
                    ...
                    if not self._automatic_maps:
                        ...
                        try:
                            ...
                            if errno == 2:
                                ...
                                warning("Unexpected error while reading secondary map file (ERRNO: %i)." % (errno),
                                        rehash=True)
                                return False
